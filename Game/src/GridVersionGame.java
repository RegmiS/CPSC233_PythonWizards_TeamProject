...
//	private static final int IS_PAUSED = 1;
	...
	private Leveling scaling_algyo = new Leveling("normal", 20);
	private ArrayList<Enemy> queueList;
	...
	public void start(Stage stage) throws Exception {
		...
		queueList = scaling_algyo.returnEnemyList(1);
		...
		*/
		...
		timer = new AnimationTimer() {
			...
			public void handle(long arg0) {
				...
				else if(queueList.isEmpty()&& enemyList.isEmpty()){
					System.out.println("queue" + queueList.size());
					timer.stop();
					scaling_algyo.increaseCurrentLevel();
					int currentlevel = scaling_algyo.returnCurrentLevel();
					queueList = scaling_algyo.returnEnemyList(currentlevel+1);
					System.out.println("queue2" + queueList.size());
					start.setVisible(true);
					
					
//					timer.stop(); // later will show the start button appear and that will start timer
				}
//				if (framecount % 100 == 0) 
					//Main.drawGame();
...
//		timer.start();
	...
	public static void addEnemy(Enemy e1) {
		enemyList.add(e1);
	}
